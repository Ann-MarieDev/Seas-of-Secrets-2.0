...
if setup == false
	...
	for(var p = 0; p < page_number; p++)
		...
		//setting individual characters anf finding and finding where the lines of text should break
		for (var c = 0; c < text_length[p]; c++)
			{
			
			var _char_pos = c+1;
			
			//store all individual characters in the "char" array
			...
			if _current_txt_w - line_break_offset[p] > line_width
				...
				line_break_offest[p] = string_width( _txt_up_to_last_space ) - string_width( _last_free_space_string );
				}
		...
		for (var c = 0; c < text_length[p]; c++)
			...
			for (var lb = 0; lb < line_break_num[p]; lb++)
				{
				///if the currentl looping character is after a line break
				if _char_pos >= line_break_pos[lb, p]
					...
					_txt_line = lb+1;// +1 bc lb starts at 0
					
					}
		
				}
			//add to the x and y cordinates base on the new info
...
if draw_char == text_length[page] && page == page_number -1
	...
	for (var op = 0; op < option_number; op++)
		...
		if option_pos == op
			{
			draw_sprite(Spr_Textbox_Star, 0, _txtb_x, _txtb_y - _op_space*option_number + _op_space*op);
			}
